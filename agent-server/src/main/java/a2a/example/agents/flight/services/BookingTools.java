package a2a.example.agents.flight.services;

import java.time.LocalDate;
import java.util.function.Function;

import a2a.example.agents.flight.data.BookingStatus;
...
public class BookingTools {
	...
	public Function<BookingDetailsRequest, BookingDetails> getBookingDetails() {
		return request -> {
			try {
				logger.info("Get booking details: {}", request);
				BookingDetails bookingDetails = flightBookingService.getBookingDetails(request.bookingNumber(), request.name());
	...
	public Function<ChangeBookingDatesRequest, String> changeBooking() {
		return request -> {
			logger.info("Change booking: {}", request);
